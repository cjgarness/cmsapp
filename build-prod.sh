...
echo ""
echo "Setting up automated database backups via cron..."
echo "Creating cron entry for production database backups..."

# Get the absolute path to the script
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BACKUP_SCRIPT="${SCRIPT_DIR}/backup-db-prod.sh"

# Make backup script executable
chmod +x "$BACKUP_SCRIPT"

# Create a cron entry for database backups
# Run backups daily at 02:00 AM
CRON_JOB="0 2 * * * $BACKUP_SCRIPT >> /var/log/cmsapp/backup.log 2>&1"

# Check if cron job already exists
if crontab -l 2>/dev/null | grep -q "$BACKUP_SCRIPT"; then
    echo "✓ Cron job already configured for database backups"
else
    echo "✓ Adding cron job for database backups..."
    # Add the cron job
    (crontab -l 2>/dev/null || true; echo "$CRON_JOB") | crontab -
    echo "✓ Cron job installed: Daily backups at 2:00 AM"
fi
